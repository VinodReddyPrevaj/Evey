...
@objc open class PeekView: UIView {
    ...
    func configureView(viewController: UIViewController, subviewFrame: CGRect, shouldHideStatusBar: Bool, options: [PeekViewAction]?=nil, completionHandler: ((Int) -> Void)?=nil, dismissHandler: (()->Void)?=nil) {
        ...
        blurEffectView.backgroundColor = UIColor(white: 1, alpha: 0.5)
        blurEffectView.frame = self.bounds
        
        let vibrancyEffect = UIVibrancyEffect(blurEffect: blurEffect)
        let vibrancyEffectView = UIVisualEffectView(effect: vibrancyEffect)
        vibrancyEffectView.backgroundColor = UIColor(white: 1, alpha: 0.5)
        ...
        if let options = options {
            ...
            for index in 0..<options.count {
                ...
                button.tag = index
                button.tintColor = #colorLiteral(red: 0.1137254902, green: 0.462745098, blue: 0.7333333333, alpha: 1)
                //button.titleLabel?.font = UIFont(name: ".SFUIText-Semibold", size: 18.0)
